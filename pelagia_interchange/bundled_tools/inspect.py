...
def main():
    ...
    result={"format":manifest.get("format"),"format_version":manifest.get("format_version"),"schema_version":manifest.get("schema_version"),"dataset_uuid":manifest.get("dataset_uuid"),"state":manifest.get("state"),"title":metadata.get("dataset",{}).get("title"),"collection":metadata.get("collection",{}),"instruments":metadata.get("instruments",[]),"streams":metadata.get("streams",[]),"acquisition_segments":len(manifest.get("acquisition_segments",[])),"shards":len(manifest.get("shards",[])),"total_frames":sum(x.get("frame_count",0) for x in manifest.get("shards",[])),"encoded_image_bytes":encoded,"package_bytes":sum(x.stat().st_size for x in root.rglob("*") if x.is_file()),"storage_distribution":codecs,"status_distribution":status,"validation":manifest.get("validation",{})}
