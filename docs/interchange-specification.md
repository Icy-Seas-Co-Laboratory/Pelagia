# Scientific Image Interchange Format 0.2
...
| Component | Authority |
|---|---|
| `data/*.sqlite` | canonical retained image payloads and frame/acquisition provenance |
...
`calibration/` contains ordinary files with manifest inventory records and descriptions. No calibration file format is mandated. `preview/` contains non-authoritative derivatives; an index SHOULD map each derivative to its authoritative stream UUID, acquisition-segment UUID, retained frame ID, and acquisition frame number. Preview generation or failure MUST NOT alter authoritative frame payloads.

## 3. Identifiers and names

Dataset, acquisition segment, deployment, instrument, stream, and shard identities MUST use UUIDs. Human-readable names MUST NOT be interpreted as persistent identity. Shard filenames are deterministic locators, not identity. The reference naming pattern is `{stream_slug}_{sequence:06d}.sqlite`.

## 4. Versions and compatibility

This is a pre-1.0 format: readers MUST require the exact supported `format_version` (`0.2`) and `schema_version` (`2`). Library semantic versions are independent of format versions. A conversion to a future incompatible pre-1.0 version MUST write a new package; finalized shards are never migrated in place.

## 5. Manifest schema

The UTF-8 JSON root MUST contain `format`, `format_version`, `schema_version`, `dataset_uuid`, `created_at`, `state`, `shards`, `acquisition_segments`, `software`, and `validation`. State is one of `building`, `finalizing`, `complete`, `verified`, or `modified`.
...
Each acquisition-segment record MUST contain `acquisition_segment_id`, `acquisition_segment_uuid`, `segment_name`, and `acquisition_mode`. `acquisition_mode` is `direct_frame_capture` or `imported_video`. It MAY contain `expected_frame_count`, capture configuration, and start/end times. Only `imported_video` may carry `import_provenance`; this is descriptive migration provenance, never an authoritative source of image bytes. Machine-local acquisition paths MUST NOT be written by direct-capture producers.
...
Each shard MUST be an ordinary standalone SQLite 3 file with tables `frames`, `acquisition_segments`, `storage_formats`, and `shard_metadata`. It MUST pass `PRAGMA integrity_check`, have no required WAL/journal sidecars when finalized, and use no nonstandard SQLite extension.

`frames.frame_id` is an INTEGER PRIMARY KEY. Rows also contain acquisition-segment identity/number, timestamp and timestamp-provenance fields, normalized storage ID, nullable BLOB, byte size, dimensions, typed stored-BLOB and optional decoded-pixel hashes, and status. `(acquisition_segment_id, acquisition_frame_number)` MUST be unique within a shard. `byte_size` MUST equal the exact BLOB length or zero for null.
...
`acquisition_segments` contains the segment identity and capture/import context used by that shard. `storage_formats` contains codec, codec version, quality, pixel format, bit depth, encoder/version, canonical parameters JSON, and a description. A writer MUST NOT repeat a prose encoding label in every frame.
...
The schema-2 columns are normative:

- `frames`: `frame_id INTEGER PRIMARY KEY`; required `acquisition_segment_id`, `acquisition_frame_number`, `byte_size`, `status`; optional `timestamp_ns`, `source_timestamp_ns`, `timestamp_source`, `clock_source`, `timezone`, `utc_conversion`, `timestamp_precision_ns`, `synchronization_method`, `known_offset_ns`, `known_drift_ppb`; required Boolean `interpolated`; optional foreign-key `storage_id`; nullable `blob`; optional `width`, `height`, `hash`, `hash_algorithm`, `decoded_pixel_hash`, `decoded_pixel_hash_algorithm`.
- `acquisition_segments`: package-local `acquisition_segment_id`, required unique `acquisition_segment_uuid`, required `segment_name` and `acquisition_mode`, optional `expected_frame_count`, capture configuration JSON, start/end times, and imported-video provenance JSON.
...
Schema 2 MUST provide indexes on `(acquisition_segment_id, acquisition_frame_number)` and `timestamp_ns`. Foreign-key relationships are `frames.acquisition_segment_id → acquisition_segments.acquisition_segment_id` and `frames.storage_id → storage_formats.storage_id`.
